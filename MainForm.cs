...
namespace Windows_Task_Dialog_Generator
{
    public partial class MainForm : Form
    {
        ...
        private void CreateAndShowDialog()
        {
            ...
            {
                TaskDialogIcon? customIcon = GetCustomTaskDialogIconFromPath(
                    textBoxCustomIconMainPath.Text
                );
            ...
            else
            {
                page.Created += (sender, e) =>
                    UpdateTitlebarIcon_OnCreated(sender, chosenTitlebarIcon);
            }
        ...
        private static void UpdateMainIcon_OnCreated(object? sender, int chosenIconID)
        {
            ...
            if (dialog != null)
            {
                ...
                try
                {
                    ...
                    SendMessage(
                        hwnd,
                        (uint)TDM.UPDATE_ICON,
                        UIntPtr.Zero,
                        new IntPtr(chosenIconID)
                    );
                }
                catch (Exception ex)
                {
        ...
        private static Icon? GetCorrespondingIconFromTaskDialogIcon(TaskDialogIcon? icon)
        {
            ...
            if (icon == TaskDialogIcon.Information)
            {
                iconID = (int)ImageresIconIDs.Information;
            }
            else if (icon == TaskDialogIcon.Warning)
            {
                iconID = (int)ImageresIconIDs.Warning;
            }
            else if (icon == TaskDialogIcon.Error)
            {
                iconID = (int)ImageresIconIDs.Error;
            }
            else if (icon == TaskDialogIcon.Shield)
            {
                iconID = (int)ImageresIconIDs.Shield;
            }
            else if (icon == TaskDialogIcon.ShieldBlueBar)
            {
                iconID = (int)ImageresIconIDs.Shield;
            }
            else if (icon == TaskDialogIcon.ShieldGrayBar)
            {
                iconID = (int)ImageresIconIDs.Shield;
            }
            else if (icon == TaskDialogIcon.ShieldSuccessGreenBar)
            {
                iconID = (int)ImageresIconIDs.ShieldSuccess;
            }
            else if (icon == TaskDialogIcon.ShieldWarningYellowBar)
            {
                iconID = (int)ImageresIconIDs.ShieldWarning;
            }
            else if (icon == TaskDialogIcon.ShieldErrorRedBar)
            {
                iconID = (int)ImageresIconIDs.ShieldError;
            }
        ...
        private Icon? DetermineChosenTitlebarIconFromSelection()
        {
            ...
            if (rbIconTitleNone.Checked)
                icon = null;
            else if (rbIconTitleInformation.Checked)
                icon = GetIconObjectFromImageRes((int)ImageresIconIDs.Information);
            else if (rbIconTitleWarning.Checked)
                icon = GetIconObjectFromImageRes((int)ImageresIconIDs.Warning);
            else if (rbIconTitleError.Checked)
                icon = GetIconObjectFromImageRes((int)ImageresIconIDs.Error);
            else if (rbIconTitleShield.Checked)
                icon = GetIconObjectFromImageRes((int)ImageresIconIDs.Shield);
            else if (rbIconTitleShieldBlueBar.Checked)
                icon = GetIconObjectFromImageRes((int)ImageresIconIDs.Shield);
            else if (rbIconTitleShieldGrayBar.Checked)
                icon = GetIconObjectFromImageRes((int)ImageresIconIDs.Shield);
            else if (rbIconTitleShieldSuccess.Checked)
                icon = GetIconObjectFromImageRes((int)ImageresIconIDs.ShieldSuccess);
            else if (rbIconTitleShieldWarning.Checked)
                icon = GetIconObjectFromImageRes((int)ImageresIconIDs.ShieldWarning);
            else if (rbIconTitleShieldError.Checked)
                icon = GetIconObjectFromImageRes((int)ImageresIconIDs.ShieldError);
        ...
        private static string GetPathFromBrowseDialog()
        {
            ...
            {
                Filter =
                    "Image files (*.ico;*.bmp;*.gif;*.jpg;*.jpeg;*.png;*.tiff)|*.ico;*.bmp;*.gif;*.jpg;*.jpeg;*.png;*.tiff|Icon files (*.ico)|*.ico|Icon From Exe (*.exe)|*.exe|All files (*.*)|*.*",
            };
        ...
        private static Icon? GetCustomIconFromPath(string filePath)
        {
            ...
            if (
                Path.GetExtension(filePath)
                    .Equals(".ico", StringComparison.CurrentCultureIgnoreCase)
            )
            {
                try
                {
            ...
            else
            {
                ...
                {
                    MessageBox.Show(
                        "The file is not a valid image. Must be one of the following: ICO, BMP, GIF, JPG, PNG or TIFF"
                    );
                    return null;
                }
                catch (Exception ex)
        ...
        private void SetTheme()
        {
            ...
            {
                case "System":
                    Theme = ThemeProvider.IsSystemDarkTheme()
                        ? ThemeProvider.DarkThemeColors
                        : ThemeProvider.LightThemeColors;
                    break;
